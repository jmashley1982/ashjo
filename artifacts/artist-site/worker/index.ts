/**
 * Cloudflare Worker that serves www.ashjo.com.
 *
 * Static files come from the `dist/static` build via the ASSETS binding; three
 * JSON endpoints are handled here. Security headers are applied to everything
 * this Worker returns.
 *
 * This file was reconstructed from the deployed `ashjo` Worker bundle — the
 * original source was never committed. Behaviour matches the deployed script.
 */
...
// Shown if the live playlist fetch fails. Keep in sync with FALLBACK_VIDEO_IDS
...
async function handleYoutubeFeed(env: Env): Promise<Response> {
  try {
    ...
    // The first <title> is the playlist itself, so titles are offset by one.
    const videos = videoIdMatches.slice(0, 3).map((m, i) => ({
      id: m[1],
      title: decodeXmlEntities(
        titleMatches[i + 1]?.[1] ?? FALLBACK_VIDEOS[i]?.title ?? `Video ${i + 1}`,
      ),
    }));
...
async function handleInstagramFeed(env: Env): Promise<Response> {
  ...
  try {
    ...
    const data = (await response.json()) as {
      data?: { media_type?: string; media_url?: string; caption?: string }[];
    };
